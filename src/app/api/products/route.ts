    ...
    try {
      ...
      const product = await prisma.product.update({
        where: { 
          ...
          tenantId: user.tenantId // Ensure only products from this tenant can be updated
        },
        data: validatedData
      })
  ...
  return withAuth(request, async (user) => {
    try {
      ...
      const allBookings = await prisma.booking.findMany({
        where: {
          tenantId: user.tenantId, // Only check bookings from this tenant
          items: {
            some: {
              productId: productId
            }
      ...
      await prisma.product.delete({
        where: { 
          id: productId,
          tenantId: user.tenantId // Ensure only products from this tenant can be deleted
        }
      })
