...
export function EditProductForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.rentPrice < 0)
      newErrors.rentPrice = "Rent price must be non-negative";
    if (formData.delayInHours < 0)
      newErrors.delayInHours = "Delay hours must be non-negative";
    ...
    try {
      const response = await apiPut("/api/products", {
        id: product.id,
        ...formData,
      });
