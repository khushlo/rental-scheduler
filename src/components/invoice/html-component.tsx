...
export const InvoiceHTMLComponent: React.FC<InvoiceHTMLComponentProps> = ({
  invoiceData,
}) => {
  const {
    booking,
    ...
    <>
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-6 sm:mb-8 space-y-4 sm:space-y-0">
        ...
        <div className="text-left sm:text-right flex-1">
          ...
          <p className="text-sm text-gray-600">
            📞 {booking.customer?.phone1 || "N/A"}
          </p>
          {booking.customer?.email &&
            booking.customer.email !== booking.customer.phone1 && (
              <p className="text-sm text-gray-600">
      ...
      <div className="mb-6 sm:mb-8 p-4 sm:p-6 bg-gray-50 rounded-lg">
        ...
        <div className="space-y-2 sm:space-y-3">
          <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-4">
            <span className="font-medium text-gray-700 text-sm sm:text-base min-w-[120px]">
              Rental Period:
            </span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-4">
            <div className="flex items-center gap-2 sm:gap-4">
              ...
              <span
                className={`px-2 sm:px-3 py-1 text-xs sm:text-sm rounded-full ${statusClass}`}
              >
        ...
        <div className="overflow-x-auto border border-gray-200 rounded-lg">
          <table className="w-full min-w-full">
            <thead className="bg-gray-50">
              <tr>
                <th className="px-3 sm:px-6 py-3 sm:py-4 text-left font-semibold text-gray-700 text-xs sm:text-sm">
                  Item
                </th>
                <th className="px-3 sm:px-6 py-3 sm:py-4 text-center font-semibold text-gray-700 text-xs sm:text-sm">
                  Qty
                </th>
                <th className="px-3 sm:px-6 py-3 sm:py-4 text-right font-semibold text-gray-700 text-xs sm:text-sm">
                  Rate (₹)
                </th>
                <th className="px-3 sm:px-6 py-3 sm:py-4 text-right font-semibold text-gray-700 text-xs sm:text-sm">
                  Amount (₹)
            ...
            <tbody className="divide-y divide-gray-200">
              {(booking.items || []).map((item: BookingItem, index: number) => (
                ...
                >
                  ...
                  <td className="px-3 sm:px-6 py-3 sm:py-4 text-gray-800 text-center text-xs sm:text-sm">
                    {item.quantity || 0}
                  </td>
                  <td className="px-3 sm:px-6 py-3 sm:py-4 text-gray-800 text-right text-xs sm:text-sm">
                    ₹{(item.pricePerDay || 0).toLocaleString()}
                  </td>
                  <td className="px-3 sm:px-6 py-3 sm:py-4 text-gray-800 text-right font-medium text-xs sm:text-sm">
        ...
        <div className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg p-4 sm:p-6">
          ...
          <div className="space-y-2 sm:space-y-3">
            <div className="flex justify-between items-center">
              ...
              <span className="font-medium text-base sm:text-lg text-gray-800">
      ...
      <div className="mt-8 sm:mt-12 pt-4 sm:pt-6 border-t border-gray-200 text-center text-gray-500">
        <p className="text-xs sm:text-sm">
          Thank you for choosing {storeName}! | For any queries, contact us at{" "}
          {storePhone} or {storeEmail}
        </p>
      </div>
    </>
  );
